...
namespace eval tcc4tcl {
	...
	proc _go {handle {outputOnly 0}} {
        ...
        append code "static Tcl_Interp*  mod_[string totitle $packageName]_interp;\n"
        set name "loot_interp"
        set adefs {Tcl_Interp* interp}
        set rtype int
        set body {
            append code "  mod_[string totitle $packageName]_interp = interp;\n"
            return 1;
        }
...
proc tcc4tcl::tclwrap {name {adefs {}} {rtype void}} {
	...
	if {$hasInterp} {
        ...
        # nameing convention:
        # mod_[string totitle $packageName]_interp
        append cbody "$rtype2 $wname\(\n"
        append cbody "    Tcl_Interp* ip = mod_[string totitle $packageName]_interp;\n"
    }
